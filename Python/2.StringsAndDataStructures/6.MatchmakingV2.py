...
for idx, p in enumerate(people):
    ...
    for index in range(idx, len(people)):
        temp = people[index]
        if (p['name'] != temp['name']) and (p['gender'] != temp['gender']):
            commonInterests = set(interests) & set(temp['interests'])
            if len(commonInterests) > 0:
                print("{} and {}: Common interests are {}".format(p['name'], temp['name'],
                                                              set(interests) & set(temp['interests'])))
