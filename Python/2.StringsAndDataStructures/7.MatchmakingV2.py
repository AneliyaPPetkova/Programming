"""
Match people by their interest
"""
people = [
    {
        'name': "Мария",
...
for idx, p in enumerate(people):
    ...
    for index in range(idx, len(people)):
        temp = people[index]
        if (p['name'] != temp['name']) \
                and (p['gender'] != temp['gender']) \
                and ((p['name'] in temp['ex']) == False) \
                and (abs(p['age'] - temp['age']) <= 6):
            commonInterests = list()
            for i in interests:
                if i in temp['interests']:
                    commonInterests.append(i)
            if len(commonInterests) > 0:
                print("{} ({}) and {} ({}): Common interests are {}"
                      .format(p['name'], p['age'], temp['name'], temp['age'], commonInterests))
